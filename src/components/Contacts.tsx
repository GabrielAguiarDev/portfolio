    ...
    <section id="contacts" className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div ref={elementRef} className={`scroll-fade-in ${isVisible ? 'visible' : ''}`}>
          <h2 className="text-4xl md:text-5xl font-bold mb-4">{translate("contacts")}</h2>
          <p className="text-muted-foreground mb-12 max-w-3xl">{translate("descriptionContact")}</p>
        </div>
          ...
          <div className={`bg-card border border-border rounded-lg p-8 scroll-fade-in ${isVisible ? 'visible' : ''}`}>
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-muted-foreground mb-4">{translate("descriptionLetsTalk")}</p>
                <Button className="bg-primary hover:bg-primary/90 gap-0">
                  <FaWhatsapp className="mr-2 w-10 h-10" />
                  {translate("letsTalk")}
                </Button>
              </div>
            </div>
